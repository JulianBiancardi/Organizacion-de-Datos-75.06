...
def preprocesado1(X):
    X_aux = X.copy()
    X_aux = reduce_by_frequency_occurrence(X_aux, ['barrio', 'religion','categoria_de_trabajo','estado_marital'], thresholds=[1000,2000,3000,5000], values=['otro','otra','otra','otro'])
    X_aux.drop(columns=['educacion_alcanzada'],inplace=True)
    
    return apply_OHE(X_aux)
