...
def discretizar_columna(df, name_col, bins=5, strat='kmeans'):
    enc = KBinsDiscretizer(n_bins=bins, encode='ordinal', strategy=strat)
    df_aux = df[[name_col]].dropna().reset_index(drop=True)
    X_binned = enc.fit_transform(df_aux)
    X_binned = pd.DataFrame(X_binned.astype(int), columns=[name_col + "_bins"])
    df = pd.concat([df, X_binned], axis=1)
